...
class PyKinectRuntime(object):
    ...
    def __init__(self, frame_source_types):
        ...
        self.long_exposure_infrared_frame_desc = self._long_exposure_infrared_source.FrameDescription
        ...
        if(self.frame_source_types & FrameSourceTypes_LongExposureInfrared):
            ...
            self._long_exposure_infrared_frame_data_capacity = ctypes.c_uint(self.long_exposure_infrared_frame_desc.Width * self.long_exposure_infrared_frame_desc.Height)
        ...
        self._last_body_frame_error = ""
        self._last_body_frame_tracked_count = 0
        self._last_body_frame_tracking_ids = []
    ...
    def close(self):
        if self._sensor is not None:
            ...
            self._long_exposure_infrared_frame_reader = None
            ...
            self._long_exposure_infrared_source = None
    ...
    def get_last_long_exposure_infrared_frame(self):
        with self._long_exposure_infrared_frame_lock:
            if self._long_exposure_infrared_frame_data is not None:
                ...
                self._last_long_exposure_infrared_frame_access = time.clock()
                return data
            else:
                return None
        ...
        try:
            ...
            try: 
                with self._body_frame_lock:
                    ...
                    self._last_body_frame_tracked_count = self._body_frame_bodies.tracked_count
                    self._last_body_frame_tracking_ids = self._body_frame_bodies.tracking_ids
                    self._last_body_frame_error = ""
                ...
                self._last_body_frame_error = repr(exc)
    ...
    def __init__(self, bodyFrame, body_frame_data, max_body_count):
        ...
        if bodyFrame is not None:
            ...
            for i in range(0, max_body_count):
               ...
               if self.bodies[i].is_tracked:
                   self.tracked_count += 1
                   self.tracking_ids.append(self.bodies[i].tracking_id)

    def copy(self):
        res = KinectBodyFrameData(None, None, 0)
        res.floor_clip_plane = self.floor_clip_plane
        res.bodies = numpy.copy(self.bodies)
        res.tracked_count = self.tracked_count
        res.tracking_ids = list(self.tracking_ids)
        return res 
